...
/**
 * The standard deviation plug in for a given channel considering its ROI
 * @author faroqal-tam
 */
public class STDev implements SimpleChannelMathPlugin {
        ...
        public double calc(DoubleChannel inChannel, double... param) {
                ...
                double mean= new Mean().calc(inChannel, param);
                ...
                for (int row=r; row < rows; row++) {
                     for (int col=c; col < cols; col++) {
                         sum+=Math.pow(inChannel.get(row, col, -1)-mean,2);
                     }
                }                                
                // case 2: non rectangular ROI
                
                return Math.sqrt(sum/(rows*cols));
        }
