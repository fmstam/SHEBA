 ...
 * @author faroqal-tam
 */
public class IterativeBiasRemover_Cri_PB extends PolynomialBiasRemover {
    ...
    public IterativeBiasRemover_Cri_PB(int polynomialDegree, double downSamplingFactor, boolean saveIfieldImage, double sampleWidthFactor, boolean darkBackground, String outputPath){
    ...
    public DoubleImage filter(DoubleImage inImage, Object... params) {
       ...
       int thetaLength = (polynomialDegree + 1) * (polynomialDegree + 2) / 2;
        ...
        try {
            ...
            // iterate
            for (int it = 3; it <= thetaLength; it++) {
                // THE STEPS:
                ...
                double[] coefficients;
                double[][] surface;
                coefficients = pf.polynomialFitQRpivot_PB(pixels, polynomialDegree, it); // fit for the pixels

                surface = pf.polynomial2Dnormalized_PB(numRows, numCols, polynomialDegree, it, coefficients); // build a surface by using the coeffcients
                ...
                DoubleImage lowpass = imageFilter.filter(gimage, new channel.doublefiltersplugins.Mean(31));
                double adjr2 = imageFilter.calculate(lowpass, ifieldImage, new adjR2(), (it+1)*(it+2)/2)[0];
