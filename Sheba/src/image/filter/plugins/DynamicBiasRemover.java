 ...
 * @author faroqal-tam
 */
public class DynamicBiasRemover extends PolynomialBiasRemover {
    ...
    public DynamicBiasRemover(int polynomialDegree, double downSamplingFactor, boolean saveIfieldImage, double sampleWidthFactor, boolean darkBackground) {
    ...
    public DoubleImage filter(DoubleImage inImage, Object... params) {
        ...
        try {
            ...
            //illumniationImage = imageFilter.calculate(illumniationImage, GeneralMath.times,255);
                ...
                if (!darkBackground) {
                    ifieldImage = imageFilter.filter(ifieldImage, new Invert((255.0)));
                }
                // put in log domain
                
                // now find the background samples
                LinkedList<DoubleLabledPoint> backgroundSamples = new LinkedList<>(); // to store the pixels and their positions
                ...
                double[] coefficients = pf.polynomialFit(pixels, polynomialDegree); // fit for the pixels
                double[][] surface = pf.polynomial2D(numRows, numCols, polynomialDegree, coefficients); // build a surface by using the coeffcients
