 ...
 * @author faroqal-tam
 */
public class IterativeBiasRemover_criterion extends PolynomialBiasRemover {
    ...
    public IterativeBiasRemover_criterion(int polynomialDegree, double downSamplingFactor, boolean saveIfieldImage, double sampleWidthFactor, boolean darkBackground) {
    ...
    public DoubleImage filter(DoubleImage inImage, Object... params) {
        ...
        try {
            ...
            // add a small value to avoid log(0);
             double shift = 0.0001; // a small shifting value
              ...
              DoubleImage surfaceImage = new DoubleImage(ifieldImage); // keep copy of the original for the polynomial fitting
            ...
            for (int it =1; it <=polynomialDegree; it++) {
                ...
                for (int k = 0; k < pathes.length; k++) {
                    ...
                    for (LabeledPoint lp : points) {
                        ...
                        backgroundSamples.addFirst(new DoubleLabledPoint(ifieldImage.get(lp.y, lp.x), lp.y, lp.x));
                    }
                }
                ...
                surfaceImage.setChannel(0, new DoubleChannel(surface)); // replace the channel
                                
                ifieldImage = imageFilter.calculate(ifieldImage, GeneralMath.minus, surfaceImage);     
            }
          
                if (!darkBackground) 
                    ifieldImage = imageFilter.filter(ifieldImage, new Invert(Math.log(255.0)));
              
                illumniationImage = imageFilter.calculate(ifieldImage, GeneralMath.exp);
                // subtract the shift value
                illumniationImage = imageFilter.calculate(illumniationImage, GeneralMath.subtract, shift);
                // normalize
                illumniationImage = imageFilter.filter(illumniationImage, new Scale(1.0 / super.downSamplingFactor));
